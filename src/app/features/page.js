        ...
        <div className="w-full bg-slate-900 rounded-[2rem] border border-slate-800 shadow-2xl p-6 flex flex-col md:flex-row gap-6 text-white min-h-[350px]">
            ...
            <div className="flex-1 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <span className="text-xs font-bold uppercase tracking-wider text-blue-400">Dimension Entry</span>
                    <span className="text-[10px] bg-blue-500/10 text-blue-300 px-2.5 py-1 rounded-full font-bold">Native SUT Mode</span>
                </div>
                    ...
                    <div className="flex gap-2">
                        <div className="flex-1 bg-slate-950 rounded-xl p-2.5 border border-slate-800 flex items-center justify-between">
                            ...
                            <div className="flex flex-col gap-0.5">
                                <button onClick={() => setWidthWhole(w => w + 1)} className="text-[10px] hover:text-blue-400">▲</button>
                                <button onClick={() => setWidthWhole(w => Math.max(1, w - 1))} className="text-[10px] hover:text-blue-400">▼</button>
                            </div>
                        </div>
                        <div className="flex-1 bg-slate-950 rounded-xl p-2.5 border border-slate-800 flex items-center justify-between">
                            <span className="text-sm font-bold text-blue-400">{widthFrac}/16″</span>
                            <div className="flex flex-col gap-0.5">
                                <button onClick={() => setWidthFrac(f => (f + 1) % 16)} className="text-[10px] hover:text-blue-400">▲</button>
                                <button onClick={() => setWidthFrac(f => (f - 1 + 16) % 16)} className="text-[10px] hover:text-blue-400">▼</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        <div className="flex-1 bg-slate-950 rounded-xl p-2.5 border border-slate-800 flex items-center justify-between">
                            ...
                            <div className="flex flex-col gap-0.5">
                                <button onClick={() => setHeightWhole(h => h + 1)} className="text-[10px] hover:text-blue-400">▲</button>
                                <button onClick={() => setHeightWhole(h => Math.max(1, h - 1))} className="text-[10px] hover:text-blue-400">▼</button>
                            </div>
                        </div>
                        <div className="flex-1 bg-slate-950 rounded-xl p-2.5 border border-slate-800 flex items-center justify-between">
                            <span className="text-sm font-bold text-blue-400">{heightFrac}/16″</span>
                            <div className="flex flex-col gap-0.5">
                                <button onClick={() => setHeightFrac(f => (f + 1) % 16)} className="text-[10px] hover:text-blue-400">▲</button>
                                <button onClick={() => setHeightFrac(f => (f - 1 + 16) % 16)} className="text-[10px] hover:text-blue-400">▼</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex-1 bg-slate-950 rounded-2xl p-4 border border-slate-800 flex flex-col justify-between items-center relative overflow-hidden">
                <span className="text-xs font-bold text-slate-400 uppercase">Interactive Sheet Preview</span>
                ...
                <div className="text-center">
                    <p className="text-[10px] text-slate-500">Auto-converts 1/16 fractional dimensions instantly</p>
                </div>
            </div>
        </div>
    );
...
const InventoryMockup = () => {
    ...
    const inventoryData = {
        ...
        aluminum: [
            { name: "Outer Frame section (6063)", stock: "140 Pcs", size: "12 Ft length", status: "In Stock" },
            { name: "Sliding Track Section", stock: "5 Pcs", size: "15 Ft length", status: "Low Stock" },
            { name: "Interlock Profile (Powder)", stock: "85 Pcs", size: "12 Ft length", status: "In Stock" },
        ],
        hardware: [
            { name: "Hydraulic Patch Fitting", stock: "120 Units", size: "Heavy duty", status: "In Stock" },
            { name: "Floor Spring Assembly", stock: "34 Units", size: "Double cylinder", status: "In Stock" },
            { name: "D-Handle 12 Inch Chrome", stock: "3 Units", size: "Stainless Steel", status: "Out of Stock" },
        ]
    };
    ...
    return (
        <div className="w-full bg-slate-900 rounded-[2rem] border border-slate-800 shadow-2xl p-6 flex flex-col text-white min-h-[350px]">
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {inventoryData[selectedTab].map((item, index) => (
                            <div key={index} className="bg-slate-950/50 p-3 rounded-xl border border-slate-850 flex justify-between items-center hover:border-slate-800 transition-colors">
            ...
            <div className="border-t border-slate-850 pt-3 flex justify-between items-center text-xs text-slate-500">
                ...
                <span className="flex items-center gap-1"><Plus className="w-3.5 h-3.5 text-green-400" /> Add Custom Product</span>
            </div>
        </div>
    );
};
    ...
    return (
        <div className="w-full bg-slate-900 rounded-[2rem] border border-slate-800 shadow-2xl p-6 flex flex-col text-white min-h-[350px] relative overflow-hidden">
            ...
            <div className="flex justify-between items-center border-b border-slate-850 pb-4">
                <div>
                    <h3 className="text-sm font-bold text-slate-100">Invoice KB-2402</h3>
        ...
        <div className="w-full bg-slate-900 rounded-[2rem] border border-slate-800 shadow-2xl p-6 flex flex-col text-white min-h-[350px]">
            ...
            <div className="flex-1 bg-slate-950 rounded-2xl p-4 border border-slate-850 flex flex-col justify-between">
                ...
                <div className="h-20 flex items-end gap-3 pt-4">
                    {[35, 45, 30, 65, 80, 55, 75].map((val, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-1">
                            <div className="w-full bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/30 rounded-t-lg relative group cursor-pointer transition-all duration-300" style={{ height: `${val}%` }}>
                ...
                <div className="mt-3 bg-amber-500/10 border border-amber-500/25 p-2 rounded-xl flex items-center gap-2 text-[10px] text-amber-300">
                    ...
                    <span><strong>Low stock alert:</strong> Outer Frame sliding profile is below 10 Pcs.</span>
                </div>
            </div>
        </div>
    );
...
const detailedFeatures = [
    ...
    },
    {
        ...
        description: "Generate beautiful, professional invoices designed for modern wholesale operations. Export as PDF and share directly via WhatsApp with one tap.",
        icon: FileText,
        color: "bg-purple-500",
